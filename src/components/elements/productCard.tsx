...
function ProductCard({ id, name, description, price, category, imageUrl }: props) {
    const [quantity, setQuantity] = useState(1);
    const [isAdded, setIsAdded] = useState(false);
    const { addToCart } = useCart();

    const handleAddToCart = () => {
        addToCart({ id, name, price, quantity, image: imageUrl, description, category });
        setIsAdded(true);
    ...
    return (
        <div className={`block transform transition duration-300 hover:scale-105 ${isAdded ? 'bg-blue-50' : ''}`}>
            <div className="border-2 rounded-md border-border p-3 hover:shadow-lg hover:border-blue-500 h-90 flex flex-col">
                <Link to={`/product/${id}`} className="w-full h-40 object-cover rounded-t-md">
                ...
                <div className="mt-auto flex items-center gap-2">
                    ...
                    />
                    <Button className="w-4/6 cursor-pointer" onClick={handleAddToCart}>Do koszyka</Button>
                </div>
            </div>
        </div>
    );
